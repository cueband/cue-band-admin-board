<template>
  ...
  <div class="py-4 container-fluid">
    <div class="mt-3 row" style="justify-content: center">
      <input type="email" placeholder="Write email here" v-model="email"/>
      <button class="btn text-primary mb-20" @click="sendPasswordResetEmail" style="margin-bottom: 20px">
          Reset User Password
      </button>
      <soft-alert :color="'danger'" class="alert-dismissible" v-if="isError">
        {{ error }}
      </soft-alert>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async sendPasswordResetEmail() {
      this.isError = false; 
      this.error = null;
      var result = await api.ResetPassword(this.email);
